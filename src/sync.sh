    ...
    while read -r line <&4; do
        if [ -n "${line}" ]; then
            ...
            if [ "${RETURN_STATUS}" = 1 ]; then
                _remove_job "${pid_show_job}"
            else
                total_show_job="$((total_show_job + 1))" && no_task_show_job="printf"
            fi
        fi
    done 4< "${SYNC_LIST}"
...
main() {
    ...
    if [ -z "${SELF_SOURCE}" ]; then
        ...
        { . "${UTILS_FOLDER}"/sh/common-utils.sh && . "${UTILS_FOLDER}"/common/common-utils.sh; } || { printf "Error: Unable to source util files.\n" && exit 1; }
    fi
